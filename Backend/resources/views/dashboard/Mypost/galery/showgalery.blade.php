...
<div class="container p-2">
    <div class="container p-2">
        <h1>{{ $galeri->title }}</h1>
        <p>{{ \Carbon\Carbon::parse($galeri->created_at)->locale('id')->translatedFormat('l, j F Y') }}</p>
            ...
            <div style="display: flex; flex-wrap: wrap; gap: 10px;">
                @foreach($images as $image)
                    <div style="flex: 1 1 300px; max-width: 300px;">
                        <img src="{{ asset('storage/' . $image) }}" alt="{{ $galeri->title }}" style="width: 100%; height: auto; border-radius: 5px;">
                    </div>
                @endforeach
            </div>
        @endif
        <br>
        @guest
        <a href="/gallery" class="btn btn-warning">Kembali Galeri</a>
        @endguest
        <!-- Tombol Hapus -->
        @auth
            @if (Auth::user()->role === 'admin' || $galeri->user_id === Auth::id())
